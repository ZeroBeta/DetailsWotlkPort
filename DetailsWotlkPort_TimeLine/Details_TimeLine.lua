...
addonId = addonId or "DetailsWotlkPort_TimeLine"
local addonPath = [[Interface\AddOns\]] .. (addonId or "DetailsWotlkPort_TimeLine")
local detailsPath = [[Interface\AddOns\DetailsWotlkPort]]
...
local class_icons_with_alpha = detailsPath .. [[\images\classes_small_alpha]]
...
local function CreatePluginFrames()
	...
	titleBar:SetBackdrop({edgeFile = [[Interface\Buttons\WHITE8X8]], edgeSize = 1, bgFile = detailsPath .. [[\images\background]], tileSize = 64, tile = true})
	...
	TimeLine.ToolbarButton = Details.ToolBar:NewPluginToolbarButton(TimeLine.OpenWindow, addonPath .. [[\icon]], Loc ["STRING_PLUGIN_NAME"], Loc ["STRING_TOOLTIP"], 12, 12, "TIMELINE_BUTTON", cooltip_menu)
	...
	timeLineFrame.bg1:SetTexture(detailsPath .. [[\images\background]], true)
	...
	--select Cooldowns button
		...
		headerFrame:SetBackdrop({bgFile = detailsPath .. [[\images\background]], tileSize = 64, tile = true})
	...
	local block_backdrop_onenter = {bgFile = "Interface\\Tooltips\\UI-Tooltip-Background", tile = true, tileSize = 16,
		edgeFile = detailsPath .. [[\images\border_2]], edgeSize = 8,
	...
	local SetPlayer = function(row, playerName, playerDataTable, elapsedTime, pixelPerSecond, deaths)
		...
		if (spec) then
			...
			row.icon.texture = detailsPath .. [[\images\spec_icons_normal]]
...
function TimeLine:OnEvent(_, event, ...)
	if (event == "ADDON_LOADED") then
		...
		if (AddonName == addonId or AddonName == "Details_TimeLine") then
			if (_G.Details) then
				CreatePluginFrames()
