...
addonId = addonId or "DetailsWotlkPort_ChartViewer"
...
local function CreatePluginFrames(data)
	...
	local cooltipMenu = function()
		...
		--> build the menu with the available tabs
			for index, tab in ipairs(ChartViewer.tabs) do
				...
				gameCooltip:AddIcon(addonPath .. "\\icon", 1, 1, 16, 16, 0, 1, 0, 1, "orange")
	...
	ChartViewer.ToolbarButton = Details.ToolBar:NewPluginToolbarButton(ChartViewer.OpenWindow, addonPath .. "\\icon", Loc ["STRING_PLUGIN_NAME"], Loc ["STRING_TOOLTIP"], 14, 14, "CHARTVIEWER_BUTTON", cooltipMenu)
	...
	if (event == "ADDON_LOADED") then
		...
		if (AddonName == addonId) then
			if (Details) then
				...
				local texture = addonPath .. "\\charticon"
