...
local parse_target_icon = function (targetObject, target)
	...
	if (targetObject) then
		...
		if (role) then
			...
			if (class == "PET") then
				icon2 = [[Interface\AddOns\DetailsWotlkPort\images\classes_small_alpha]]
				icon2coords = PetCoords
			elseif (class and RAID_CLASS_COLORS [class]) then
				if (targetObject.spec) then
					icon2 = [[Interface\AddOns\DetailsWotlkPort\images\spec_icons_normal_alpha]]
...
function StreamOverlay.OpenOptionsPanel (fromOptionsPanel)
	if (not StreamOverlayOptionsPanel) then
		...
		classicModeTexture:SetTexture([[Interface\Addons\DetailsWotlkPort_Streamer\images\tracker_full]])
		...
		squareModeTexture:SetTexture([[Interface\Addons\DetailsWotlkPort_Streamer\images\tracker_square]])
...
function StreamOverlay:OnEvent (_, event, ...)
	...
	if (event == "ADDON_LOADED") then
		...
		if (AddonName == "DetailsWotlkPort_Streamer" or AddonName == "Details_Streamer") then
			...
			if (_G.Details) then
				...
				if (StreamOverlay.db.is_first_run and not Details:GetTutorialCVar ("STREAMER_PLUGIN_FIRSTRUN")) then
					local show_frame = function()
						...
						youtubeTwitchIcons:SetTexture([[Interface\AddOns\DetailsWotlkPort\images\icons2]])
						...
						image1:SetTexture([[Interface\AddOns\DetailsWotlkPort_Streamer\streamer_plugin_lines]])
...
editbox:SetBackdrop ({bgFile = "Interface\\AddOns\\DetailsWotlkPort\\images\\background", tile = true, tileSize = 16, insets = {left = 0, right = 0, top = 0, bottom = 0}})
