...
addonId = addonId or "DetailsWotlkPort_DeathGraphs"
local Details = Details
local detailsFramework = DetailsFramework
local _
local addonPath = [[Interface\AddOns\]] .. addonId
...
local createPluginFunctions = function()
	...
	adlObject.ToolbarButton = adlObject.ToolBar:NewPluginToolbarButton(adlObject.OpenWindow, addonPath .. "\\icon", Loc["STRING_PLUGIN_NAME"], Loc["STRING_TOOLTIP"], 16, 16, "DEATHGRAPHICS_BUTTON", onEnterIconCooltipMenu)
	...
	if (event == "ADDON_LOADED") then
		...
		if (AddonName == addonId) then
			if (Details) then
				...
				local install, saveddata = _G.Details:InstallPlugin("TOOLBAR", "Advanced Death Logs", addonPath .. "\\icon", adlObject, "DETAILS_PLUGIN_DEATH_GRAPHICS", MINIMAL_DETAILS_VERSION_REQUIRED, "Details! Team", adlObject.version_string, defaults)
