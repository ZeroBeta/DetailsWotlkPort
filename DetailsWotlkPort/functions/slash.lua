...
--		/run TTT:SetTexture("Interface\\1024.tga")
	...
	elseif (msg == "chaticon") then
		Details:Msg("|TInterface\\AddOns\\DetailsWotlkPort\\images\\icones_barra:" .. 14 .. ":" .. 14 .. ":0:0:256:32:0:32:0:32|tteste")
	...
	elseif (msg == "slider") then
		...
		f:SetWidth(300)
		f:SetHeight(150)
		f:SetBackdrop({bgFile = "Interface\\AddOns\\DetailsWotlkPort\\images\\background", tile = true, tileSize = 16, insets = {left = 0, right = 0, top = 0, bottom = 0}})
		...
		for i = 1, 7 do
			...
			row:SetWidth(200)
			row:SetHeight(20)
			row:SetBackdrop({bgFile = "Interface\\AddOns\\DetailsWotlkPort\\images\\background", tile = true, tileSize = 16, insets = {left = 0, right = 0, top = 0, bottom = 0}})
...
--	https://www.dropbox.com/s/ulyeqa2z0ummlu7/1024.tga?dl=0
			...
			a:SetScript("OnUpdate", function(self, deltaTime)
				...
				texture:SetTexture([[Interface\AddOns\DetailsWotlkPort\images\options_window]])
	...
	elseif (msg == "alert") then
		...
		if (lower_instance) then
			...
			if (instance) then
				...
				instance:InstanceAlert ("Boss Defeated, Open History! ", {[[Interface\AddOns\DetailsWotlkPort\images\icons]], 16, 16, false, 434/512, 466/512, 243/512, 273/512}, 40, func, true)
			end
		end
	...
	elseif (msg == "qq") then
		...
		--local icon = {[[Interface\AddOns\DetailsWotlkPort\images\icons]], 16, 16, false, 434/512, 466/512, 243/512, 273/512}
...
if (WOW_PROJECT_ID == WOW_PROJECT_MAINLINE) then
	...
	for i = 1, 30 do
		...
		detailsFramework:SetMask(teleportButton.Icon, [[Interface\AddOns\DetailsWotlkPort\images\masks\portal_mask.tga]])
		...
		if (openRaidLib) then
			if (not DetailsKeystoneInfoFrame.Created) then
				...
				local mainTabSelectorRadioOptions = {{
						...
						param = "details",
						get = function()end,
						texture = [[Interface\AddOns\DetailsWotlkPort\images\minimap]],
						texcoord = {0, 1, 0, 1},
						mask = nil,
						width = 20,
						height = 20,
...
--]=]
				...
				local createLineForScroll = function(self, index)
					...
					teleportButton:SetBackdrop({bgFile = "Interface\\AddOns\\DetailsWotlkPort\\images\\background", tile = true, tileSize = 16, insets = {left = 0, right = 0, top = 0, bottom = 0}})
					...
					detailsFramework:SetMask(blockTeleporterButton.Icon, [[Interface\AddOns\DetailsWotlkPort\images\masks\portal_mask.tga]])
