...
function immersionFrame.CheckIfCanEnableImmersion()
    ...
    elseif GetRealZoneText then
        -- fallback: use zone text hash to mimic map names
        mapID = GetCurrentMapAreaID and GetCurrentMapAreaID() or nil
    end
    if (mapID) then
        ...
        if C_Map.GetMapInfo then
            ...
            mapFileName = info and info.name
        end
...
local iconPath1 = [[Interface\AddOns\DetailsWotlkPort\images\special_bar_icons]]
