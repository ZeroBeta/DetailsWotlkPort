...
do
	...
	function Details:GetUnknownClassIcon()
		return [[Interface\AddOns\DetailsWotlkPort\images\classes_small]], unpack(CONST_UNKNOWN_CLASS_COORDS)
	end
	...
	function Details:GetIconTexture(iconType, bWithAlpha)
		...
		if (iconType == "spec") then
			if (bWithAlpha) then
				return [[Interface\AddOns\DetailsWotlkPort\images\spec_icons_normal_alpha]]
			else
				return [[Interface\AddOns\DetailsWotlkPort\images\spec_icons_normal]]
			end
		else --if is class
			if (bWithAlpha) then
				return [[Interface\AddOns\DetailsWotlkPort\images\classes_small_alpha]]
			else
				return [[Interface\AddOns\DetailsWotlkPort\images\classes_small]]
			end
		end
	end
	...
	function Details:GetRoleIcon(role)
		return [[Interface\AddOns\DetailsWotlkPort\images\icons2]], unpack(roles[role])
	end
	...
	function Details:GetClassIcon(class)
		...
		elseif (class == "PET") then
			return [[Interface\AddOns\DetailsWotlkPort\images\classes_small]], 0.25, 0.49609375, 0.75, 1

		else
			return [[Interface\AddOns\DetailsWotlkPort\images\classes_small]], unpack(Details.class_coords[class])
		end
	end
	...
	function Details:GetSpecIcon(spec, useAlpha)
		if (not spec or spec == 0 or not Details.class_specs_coords[spec]) then
			...
			return [[Interface\AddOns\DetailsWotlkPort\images\classes_small]], unpack(Details.class_coords["UNKNOW"])
		end

		if (useAlpha) then
			return [[Interface\AddOns\DetailsWotlkPort\images\spec_icons_normal_alpha]], unpack(Details.class_specs_coords [spec])
		else
			return [[Interface\AddOns\DetailsWotlkPort\images\spec_icons_normal]], unpack(Details.class_specs_coords[spec])
		end
	end
	...
	function Details:GetPlayerIcon(playerName, combatObject)
		...
		if (playerObject) then
			...
			if (spec and Details.class_specs_coords[spec]) then
				texturePath = [[Interface\AddOns\DetailsWotlkPort\images\spec_icons_normal]]
				left, right, top, bottom = unpack(Details.class_specs_coords[spec])

			elseif (playerObject.classe) then
				texturePath = [[Interface\AddOns\DetailsWotlkPort\images\classes_small]]
