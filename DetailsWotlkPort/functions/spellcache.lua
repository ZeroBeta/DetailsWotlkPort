...
do
	...
	elseif (DetailsFramework.IsShadowlandsWow()) then
		defaultSpellCustomization = {
			...
			[108271] = {name = GetSpellInfo(108271), icon = "Interface\\Addons\\DetailsWotlkPort\\images\\icon_astral_shift"},
			[196917] = {name = lightOfTheMartyr_Name .. " (" .. Loc ["STRING_DAMAGE"] .. ")", icon = lightOfTheMartyr_Icon},
			[77535] = {name = GetSpellInfo(77535), icon = "Interface\\Addons\\DetailsWotlkPort\\images\\icon_blood_shield"},
	...
	elseif (DetailsFramework.IsTWWWow()) then
		defaultSpellCustomization = {
			...
			[196917] = {name = lightOfTheMartyr_Name .. " (" .. Loc ["STRING_DAMAGE"] .. ")", icon = lightOfTheMartyr_Icon},
			[77535] = {name = GetSpellInfo(77535), icon = "Interface\\Addons\\DetailsWotlkPort\\images\\icon_blood_shield"},
			[395296] = {name = GetSpellInfo(395296) .. " (on your self)", icon = "Interface\\Addons\\DetailsWotlkPort\\images\\ebon_might"},
			[424428] = {name = (GetSpellInfo(424428) or "none") .. " (4P)", icon = "Interface\\Addons\\DetailsWotlkPort\\images\\spells\\eruption_tier4.jpg", defaultName = GetSpellInfo(424428), breakdownCanStack = true}, --augmentation 4pc tier 10.2
			[422779] = {name = (GetSpellInfo(422779) or "none") .. " (4P)", icon = "Interface\\Addons\\DetailsWotlkPort\\images\\spells\\burning_frenzy_tier4.jpg", defaultName = GetSpellInfo(422779)}, --feral 4pc tier 10.2
		}
	...
	elseif (DetailsFramework.IsPandaWow()) then
		...
		--10.2
		customItemList[426672] = {itemId = 207168, isPassive = true, nameExtra = "(vers)", icon = [[Interface\AddOns\DetailsWotlkPort\images\spells\spell_druid_bearhug_blackwhite.jpg]]} --trinket: Pip's Emerald Friendship Badge urctos
		...
		customItemList[426676] = {itemId = 207168, isPassive = true, nameExtra = "(crit)", icon = [[Interface\AddOns\DetailsWotlkPort\images\spells\elf_face_right.jpg]]} --trinket: Pip's Emerald Friendship Badge aerwynn
		...
		customItemList[426647] = {itemId = 207168, isPassive = true, nameExtra = "(mast)", icon = [[Interface\AddOns\DetailsWotlkPort\images\spells\lil_dragon_left.jpg]]} --trinket: Pip's Emerald Friendship Badge pip
...
--]=]
	...
	function Details:FillUserCustomSpells()
		for spellId, spellTable in pairs(defaultSpellCustomization) do
			...
			-- fall back to the raw spellId string if GetSpellInfo isn't available on this client
			if (not spellName) then
	...
	--overwrite for API GetSpellInfo function
		Details.getspellinfo = function(spellId)
			local info = Details.spellcache[spellId]
			if not info then
				return nil
			end
			return unpack(info)
