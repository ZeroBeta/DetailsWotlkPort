...
--local pointers
	...
	--spell reflection
		...
		function classicAbsorb.RegisterShield(timestamp, sourceSerial, sourceName, sourceFlags, targetSerial, targetName, spellId, spellSchool, amount)
			if (not classicAbsorb.spellLookup[spellId] or not targetName) then
				return
			end
		...
		function classicAbsorb.ApplyAbsorb(timestamp, targetSerial, targetName, targetFlags, targetFlags2, absorbed, school)
			...
			for i = #shieldList, 1, -1 do
				...
				parser:heal("SPELL_HEAL", timestamp, entry.srcGUID, entry.srcName, entry.srcFlags, targetSerial, targetName, targetFlags, targetFlags2, entry.spellid, (_GetSpellInfo(entry.spellid) or "Absorb"), entry.school or school or 1, credit, 0, 0, nil, true)
			...
			if (remaining > 0 and lastShield) then
				parser:heal("SPELL_HEAL", timestamp, lastShield.srcGUID, lastShield.srcName, lastShield.srcFlags, targetSerial, targetName, targetFlags, targetFlags2, lastShield.spellid, (_GetSpellInfo(lastShield.spellid) or "Absorb"), lastShield.school or school or 1, remaining, 0, 0, nil, true)
			end
		...
		function classicAbsorb.HandleAura(token, timestamp, sourceSerial, sourceName, sourceFlags, targetSerial, targetName, targetFlags, targetFlags2, spellId, spellName, spellSchool, auraType, amount)
			...
			if (token == "SPELL_AURA_APPLIED" or token == "SPELL_AURA_REFRESH") then
				classicAbsorb.RegisterShield(timestamp, sourceSerial, sourceName, sourceFlags, targetSerial, targetName, spellId, spellSchool, amount)
