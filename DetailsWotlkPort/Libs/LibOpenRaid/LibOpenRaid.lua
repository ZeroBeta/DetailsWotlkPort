...
-- Ensure global tables exist on legacy clients where they aren't auto-generated
...
LIB_OPEN_RAID_COOLDOWNS = LIB_OPEN_RAID_COOLDOWNS or {}
...
--declare the library within the LibStub
    ...
    -- Early initialization of CooldownManager and GearManager so they're available for Functions.lua
    ...
    openRaidLib.GearManager = {
        UnitData = {}, --stores gear information for each player
    }

    openRaidLib.UnitInfoManager = {
        UnitData = {}, --stores unit information
    }

    -- Early initialization of internalCallback system
    ...
    openRaidLib.internalCallback.RegisterCallback = function(event, func)
        if (not openRaidLib.internalCallback.events[event]) then
            openRaidLib.internalCallback.events[event] = {}
        end
    ...
    function openRaidLib.internalCallback.TriggerEvent(event, ...)
        ...
        if (eventCallbacks) then
            for i = 1, #eventCallbacks do
                local callback = eventCallbacks[i]
                local result, error = pcall(callback, ...)
                if (not result) then
                    sendChatMessage("LibOpenRaid callback error:", event, error)
                end
            end
        end
    end
...
--~comms
    ...
    local function SupportsInstanceChat()
        return ChatTypeInfo and ChatTypeInfo.INSTANCE_CHAT ~= nil
    end

    local hasInstanceCategory = _G.LE_PARTY_CATEGORY_INSTANCE ~= nil
    local function SafeIsInGroupInstance(isRaid)
        if (not SupportsInstanceChat()) then
            return false
        end
        if (hasInstanceCategory) then
            if isRaid then
                local ok, value = pcall(IsInRaid, LE_PARTY_CATEGORY_INSTANCE)
                return ok and value
            else
    ...
    local function GetGroupChannel(isRaid)
        if (isRaid) then
            return SafeIsInGroupInstance(true) and "INSTANCE_CHAT" or "RAID"
        else
            return SafeIsInGroupInstance(false) and "INSTANCE_CHAT" or "PARTY"
        end
    end
        ...
        if (flags) then
            if (bit.band(flags, CONST_COMM_SENDTO_PARTY)) then --send to party
                if (IsInGroup() and not IsInRaid()) then
                    ---@type commdata
                    local commData = {data = dataEncoded, channel = GetGroupChannel(false), bIsSafe = bIsSafe, plainText = data}
            ...
            if (bit.band(flags, CONST_COMM_SENDTO_RAID)) then --send to raid
                if (IsInRaid()) then
                    local commData = {data = dataEncoded, channel = GetGroupChannel(true), bIsSafe = bIsSafe, plainText = data}
                    table.insert(commScheduler, commData)
                end
            end
...
--------------------------------------------------------------------------------------------------------------------------------
--~cooldowns ~cooldown
--CooldownManager is initialized early in this file (around line 86)
