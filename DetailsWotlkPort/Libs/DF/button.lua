...
function detailsFramework:CreateCloseButton(parent, frameName)
	...
	-- Wrath client doesn't give a disabled texture and may not support SetAtlas; guard accordingly.
	if not disabledTexture then
		disabledTexture = closeButton:CreateTexture(nil, "BORDER")
		closeButton:SetDisabledTexture(disabledTexture)
	end

	local function SetTextureOrAtlas(tex, atlas, fallbackTexture)
		if not tex then
			return
		end
	...
	SetTextureOrAtlas(normalTexture, "RedButton-Exit", "Interface\\Buttons\\UI-Panel-MinimizeButton-Up")
	SetTextureOrAtlas(highlightTexture, "RedButton-Highlight", "Interface\\Buttons\\UI-Panel-MinimizeButton-Highlight")
	SetTextureOrAtlas(pushedTexture, "RedButton-exit-pressed", "Interface\\Buttons\\UI-Panel-MinimizeButton-Down")
	SetTextureOrAtlas(disabledTexture, "RedButton-Exit-Disabled", "Interface\\Buttons\\UI-Panel-MinimizeButton-Disabled")
