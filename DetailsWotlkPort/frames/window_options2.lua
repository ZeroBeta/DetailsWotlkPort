...
function Details222.OptionsPanel.InitializeOptionsWindow(instance)
    ...
    DetailsOptionsWindow:SetBackdrop({bgFile = [[Interface\Buttons\WHITE8X8]], edgeFile = nil, tile = true, tileSize = 16, edgeSize = 1, insets = {left = 0, right = 0, top = 0, bottom = 0}})
    -- Backdrop corners on a frame with a border can emit a warning on Wrath; skip if a border already exists.
    if not (optionsFrame.GetBackdropBorderColor and optionsFrame:GetBackdropBorderColor()) then
        detailsFramework:AddRoundedCornersToFrame(optionsFrame, Details.PlayerBreakdown.RoundedCornerPreset)
    end
    if optionsFrame.SetColor then
        optionsFrame:SetColor(unpack(Details.frame_background_color))
    elseif optionsFrame.SetBackdropColor then
        optionsFrame:SetBackdropColor(unpack(Details.frame_background_color))
    end
	...
	--create test bars ~test
        ...
        fillbars:SetIcon ("Interface\\AddOns\\DetailsWotlkPort\\images\\icons", nil, nil, nil, {323/512, 365/512, 42/512, 78/512}, {1, 1, 1, 0.6}, 4, 2)
    ...
    --change log
        ...
        changelog:SetIcon ("Interface\\AddOns\\DetailsWotlkPort\\images\\icons", nil, nil, nil, {367/512, 399/512, 43/512, 76/512}, {1, 1, 1, 0.8}, 4, 2)
