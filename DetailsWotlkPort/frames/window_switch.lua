...
function Details222.CreateAllDisplaysFrame()
	...
	allDisplaysFrame:SetScript("OnShow", function()
		...
		if (not allDisplaysFrame.already_built) then
			...
			title_icon:SetTexture([[Interface\AddOns\DetailsWotlkPort\images\icons]])
			...
			--prepare for plugins
				...
				local title_icon = allDisplaysFrame:CreateTexture(nil, "overlay")
				title_icon:SetTexture([[Interface\AddOns\DetailsWotlkPort\images\modo_icones]])
	...
	if (IsControlKeyDown()) then
		if (not Details.tutorial.ctrl_click_close_tutorial) then
			if (not DetailsCtrlCloseWindowPanelTutorial) then
				...
				tutorialFrame:SetBackdrop({bgFile = "Interface\\AddOns\\DetailsWotlkPort\\images\\background", tile = true, tileSize = 16 })
...
function Details.switch:Update()
	...
	for i = 1, slots_shown do
		...
		if (options and options.sub_atributo) then
			if (type(options.atributo) == "number") then
				local subList = Details.sub_atributos[options.atributo] and Details.sub_atributos[options.atributo].lista
				if (not subList or not subList[options.sub_atributo]) then
					options.sub_atributo = 1
				end
			end
			if (options.atributo == 5) then --custom
				...
				if (not CustomObject) then --ele j� foi deletado
					icone = [[Interface\AddOns\DetailsWotlkPort\images\icons]]
					coords = add_coords
