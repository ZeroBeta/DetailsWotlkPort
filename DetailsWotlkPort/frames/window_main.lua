...
local function CreateFrame(frameType, name, parent, template, ...)
	...
	-- Retry without template and with capitalized frameType (3.3.5 is picky about casing)
	local safeType = type(frameType) == "string" and (frameType:sub(1,1):upper() .. frameType:sub(2):lower()) or frameType
	ok, frame = pcall(_CreateFrame, safeType, name, parent, nil, ...)
	if ok and frame then
		return frame
	end
...
local defaultBackdropSt = {
	bgFile = [[Interface\AddOns\DetailsWotlkPort\images\background]], tile = true, tileSize = 12,
...
--skins TCoords
	local DEFAULT_SKIN = [[Interface\AddOns\DetailsWotlkPort\images\skins\classic_skin]]
	...
	local menus_backdrop = {
		...
		edgeSize=1,
		bgFile = [[Interface\AddOns\DetailsWotlkPort\images\background]],
		tileSize=16,
		tile=true,
...
local function OnLeaveMainWindow(instancia, self)
	if (not instancia or not instancia.baseframe) then
		return
	end
...
local update_line = function(self, target_frame)
	...
	for i = 0, (distance/pixelsPerArrow) do
		...
		if (not ball) then
			...
			ball:SetTexture([[Interface\AddOns\DetailsWotlkPort\images\icons]])
			ball:SetSize(16, 16)
			ball:SetAlpha(0.3)
...
local BGFrame_scripts_onmousedown = function(self, button)
	if (not self._instance or not self._instance.baseframe) then
		return
	end
...
local resizeTooltip = {
	...
	{icon = [[Interface\AddOns\DetailsWotlkPort\images\key_shift]], width = 24, height = 14, l = 0, r = 1, t = 0, b =0.640625},

	{text = "+|cff33CC00 Click|cffEEEEEE: " .. Loc["STRING_RESIZE_VERTICAL"]},
	{icon = [[Interface\AddOns\DetailsWotlkPort\images\key_alt]], width = 24, height = 14, l = 0, r = 1, t = 0, b =0.640625},

	{text = "+|cff33CC00 Click|cffEEEEEE: " .. Loc["STRING_RESIZE_ALL"]},
	{icon = [[Interface\AddOns\DetailsWotlkPort\images\key_ctrl]], width = 24, height = 14, l = 0, r = 1, t = 0, b =0.640625}
}
...
local unSnapButtonTooltip = {
	...
	{icon = [[Interface\AddOns\DetailsWotlkPort\images\icons]], width = 14, height = 14, l = 160/512, r = 179/512, t = 142/512, b = 162/512},
}
...
local lineScript_Onenter = function(self)
	...
	if lefttext and lefttext.IsTruncated then
		isTruncated = lefttext:IsTruncated()
	else
		-- WotLK doesn't have IsTruncated; approximate with string width vs. frame width.
		if lefttext and lefttext.GetStringWidth and lefttext.GetWidth then
			isTruncated = (lefttext:GetStringWidth() > lefttext:GetWidth())
		end
	end
...
local iconFrame_OnEnter = function(self)
	...
	if (actor) then
		...
		elseif (actor.nome) then --ensure it's an actor table
			...
			local specId, specName, specDescription, specIcon, specRole, specClass, specL, specR, specT, specB = Details:GetActorSpecInfo(actor, class)
			...
			if (class == "UNKNOW" or class == "UNGROUPPLAYER") then
				GameCooltip:AddIcon([[Interface\AddOns\DetailsWotlkPort\images\classes_small_alpha]], 1, 1, iconSize, iconSize, 0, 0.25, 0.75, 1)
			else
			...
			GameCooltip:SetOption("StatusBarTexture", [[Interface\AddOns\DetailsWotlkPort\images\bar_skyline]])
...
local function CreateAlertFrame(baseframe, instancia)
	...
	alertBackgroundFrame:SetBackdrop({bgFile = [[Interface\AddOns\DetailsWotlkPort\images\background]], tile = true, tileSize = 16,
	insets = {left = 0, right = 0, top = 0, bottom = 0}})
...
do
	...
	icon:SetTexture([[Interface\AddOns\DetailsWotlkPort\images\minimap]])
...
function gump:CriaJanelaPrincipal(ID, instancia, criando)
	...
	titleBar.texture:SetTexture([[Interface\AddOns\DetailsWotlkPort\images\bar_serenity]])
...
-- instance mini widgets -------------------------------------------------------------------------------------------------------------------------------------------------
	...
	--details version
		...
		if (not Details222.PrivateInstanceText) then
			...
			Details222.PrivateInstanceText:SetFont("Interface\\AddOns\\DetailsWotlkPort\\Fonts\\Accidental Presidency.ttf", 10, "NONE")
...
-- break snap button ----------------------------------------------------------------------------------------------------------------------------------------------------------
			...
			if (instancia.break_snap_button) then
				...
				local normal = instancia.break_snap_button:GetNormalTexture()
				if normal then normal:SetTexCoord(unpack(COORDS_UNLOCK_BUTTON)) end
				local disabled = instancia.break_snap_button:GetDisabledTexture()
				if disabled then disabled:SetTexCoord(unpack(COORDS_UNLOCK_BUTTON)) end
				local highlight = instancia.break_snap_button:GetHighlightTexture()
				if highlight then highlight:SetTexCoord(unpack(COORDS_UNLOCK_BUTTON)) end
				local pushed = instancia.break_snap_button:GetPushedTexture()
				if pushed then pushed:SetTexCoord(unpack(COORDS_UNLOCK_BUTTON)) end
			end
...
-- side bars highlights ------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	-- If frame scaffolding failed on this client, skip highlight setup entirely.
	if not (floatingframe and baseframe and baseframe.cabecalho and baseframe.cabecalho.top_bg and baseframe.cabecalho.ball_r) then
		return
	end

	--top
	local frameHighlightTop = CreateFrame("frame", "DetailsTopSideBarHighlight" .. instancia.meu_id, floatingframe)
	if (not frameHighlightTop) then
		frameHighlightTop = floatingframe:CreateTexture("DetailsTopSideBarHighlight" .. instancia.meu_id, "overlay")
	end

	if (frameHighlightTop and frameHighlightTop.SetFrameLevel) then
		frameHighlightTop:SetFrameLevel(floatingframe:GetFrameLevel())
	end

	local createFlashOk
	if (gump.CreateFlashAnimation) then
		createFlashOk = pcall(gump.CreateFlashAnimation, gump, frameHighlightTop)
	end

	if (not createFlashOk) then
		-- fallback flash: just create a simple fade texture holder
		if frameHighlightTop.CreateAnimationGroup then
			local ag = frameHighlightTop:CreateAnimationGroup()
	...
	frameHighlightTop:Hide()
		...
		instancia.h_cima:SetTexture([[Interface\AddOns\DetailsWotlkPort\images\highlight_updown]])
	...
	--bottom
		...
		instancia.h_baixo:SetTexture([[Interface\AddOns\DetailsWotlkPort\images\highlight_updown]])
	...
	--left
		...
		instancia.h_esquerda:SetTexture([[Interface\AddOns\DetailsWotlkPort\images\highlight_leftright]])
	...
	--right
		...
		instancia.h_direita:SetTexture([[Interface\AddOns\DetailsWotlkPort\images\highlight_leftright]])
...
function gump:CreateNewLine(instance, index)
	...
	--newLine.extraStatusbar.texture:SetTexture([[Interface\AddOns\DetailsWotlkPort\images\bar_textures\bar_of_bars.png]]) --setColorTexture is very expensive, so set the color once and use vertex color to change it
...
function Details:GetBarBracket()
	local bracketTable = brackets[self.row_info.textR_bracket] or brackets["("]

	--inline fontstrings already have spacing from anchors, so trim padding to avoid extra blanks
	if (self.use_multi_fontstrings) then
		local leftBracket = bracketTable[1] or ""
		local rightBracket = bracketTable[2] or ""
		leftBracket = leftBracket:gsub("^%s+", "")
		rightBracket = rightBracket:gsub("%s+$", "")
...
function Details:GetBarSeparator()
	local separator = separators[self.row_info.textR_separator] or separators[","]
...
function Details:SetInstanceWallpaperLevel(wallpaperLevel)
	...
	if (not wallpaper.texture and not texture) then
		texture = "Interface\\AddOns\\DetailsWotlkPort\\images\\background"
		texCoord = {0, 1, 0, 0.7}
		alpha = 0.5
...
function Details:SetWindowAlphaForCombat(enteringInCombat, trueHide, alphaAmount)
	...
	if (trueHide and amount == 0) then
		if self.baseframe then self.baseframe:Hide() end
		if self.rowframe then self.rowframe:Hide() end
		if self.windowSwitchButton then self.windowSwitchButton:Hide() end
		if (Details.debug) then
	...
	if (self.show_statusbar) then
		if self.baseframe and self.baseframe.barra_fundo then
...
function gump:CriaRodape(baseframe, instancia)
	...
	bgFile = [[Interface\AddOns\DetailsWotlkPort\images\background]], tile = true, tileSize = 16,
	insets = {left = 0, right = 0, top = 0, bottom = 0}})
...
local build_mode_list = function(self, deltaTime)
	...
	if (parameters_table[2] > 0.15) then
		...
		gameCooltip:AddIcon([[Interface\AddOns\DetailsWotlkPort\images\modo_icones]], 1, 1, 20, 20, 32/256, 32/256*2, 0, 1)

		gameCooltip:AddLine(Loc["STRING_MODE_ALL"])
		gameCooltip:AddMenu(1, function() instance:SetMode(3) end)
		gameCooltip:AddIcon([[Interface\AddOns\DetailsWotlkPort\images\modo_icones]], 1, 1, 20, 20, 32/256*2, 32/256*3, 0, 1)

		gameCooltip:AddLine(Loc["STRING_OPTIONS_PLUGINS"])
		gameCooltip:AddMenu(1, function() instance:SetMode(4) end)
		gameCooltip:AddIcon([[Interface\AddOns\DetailsWotlkPort\images\modo_icones]], 1, 1, 20, 20, 32/256*3, 32/256*4, 0, 1)
		...
		gameCooltip:AddIcon([[Interface\AddOns\DetailsWotlkPort\images\modo_icones]], 1, 1, 20, 20, 0.625, 0.75, 0, 1)
		...
		gameCooltip:AddIcon([[Interface\AddOns\DetailsWotlkPort\assets\textures\icons\midnight.png]], 2, 1, 16, 14)
		if (hasClosedInstances) then
			GameCooltip:AddLine("$div", nil, 2, nil, -5, -11)
		end
		...
		gameCooltip:AddIcon([[Interface\AddOns\DetailsWotlkPort\images\modo_icones]], 1, 1, 20, 20, 0.5, 0.625, 0, 1)
				...
				if (thisCombat and not thisCombat.__destroyed) then
					...
					if (combatCategory == DETAILS_SEGMENTTYPE_MYTHICDUNGEON) then
						...
						elseif (combatType == DETAILS_SEGMENTTYPE_MYTHICDUNGEON_BOSS or combatType == DETAILS_SEGMENTTYPE_MYTHICDUNGEON_BOSSWIPE) then
							...
							local skull = "|TInterface\\AddOns\\DetailsWotlkPort\\images\\icons:16:16:0:0:512:512:496:512:0:16|t"
			...
			if (thisCombat.is_mythic_dungeon_segment) then
				...
				if (mythicDungeonInfo) then
					...
					if (isMythicOverallSegment) then
						...
						--CoolTip:AddIcon([[Interface\AddOns\DetailsWotlkPort\images\icons]], "main", "left", 14, 10, 479/512, 510/512, 24/512, 51/512)
						gameCooltip:AddLine(zoneName .. " +" .. mythicLevel .. " (" .. Loc["STRING_SEGMENTS_LIST_OVERALL"] .. ")", nil, 2, "white", "white")
...
function Details:ChangeSkin(skin_name)
	...
	do
		...
		if (not toolbar_icon_file) then
			toolbar_icon_file = [[Interface\AddOns\DetailsWotlkPort\images\toolbar_icons]]
		end
...
--close button functions
	...
	local closeButton_OnEnter = function(self)
		if (not self.instance or not self.instance.baseframe) then
			return
		end
...
local reportButton_OnEnter = function(self, motion, forced)
	...
	GameCooltip:AddIcon([[Interface\Addons\DetailsWotlkPort\Images\report_button]], 1, 1, 12, 19)
...
function gump:CriaCabecalho (baseframe, instancia)
	...
	baseframe.cabecalho.fechar:SetNormalTexture([[Interface\AddOns\DetailsWotlkPort\images\toolbar_icons]])
	baseframe.cabecalho.fechar:GetNormalTexture():SetTexCoord(160/256, 192/256, 0, 1)
	baseframe.cabecalho.fechar:SetHighlightTexture([[Interface\AddOns\DetailsWotlkPort\images\toolbar_icons]])
	baseframe.cabecalho.fechar:GetHighlightTexture():SetTexCoord(160/256, 192/256, 0, 1)
	baseframe.cabecalho.fechar:SetPushedTexture([[Interface\AddOns\DetailsWotlkPort\images\toolbar_icons]])
...
--title bar buttons
	...
	baseframe.cabecalho.modo_selecao = gump:NewButton(baseframe, nil, "DetailsModeButton"..instancia.meu_id, nil, 16, 16, modeSelector_OnClick, nil, nil, [[Interface\AddOns\DetailsWotlkPort\images\modo_icone]])
	...
	b:SetNormalTexture([[Interface\AddOns\DetailsWotlkPort\images\toolbar_icons]])
	b:GetNormalTexture():SetTexCoord(0/256, 32/256, 0, 1)
	b:SetHighlightTexture([[Interface\AddOns\DetailsWotlkPort\images\toolbar_icons]])
	b:GetHighlightTexture():SetTexCoord(0/256, 32/256, 0, 1)
	b:SetPushedTexture([[Interface\AddOns\DetailsWotlkPort\images\toolbar_icons]])
	...
	baseframe.cabecalho.segmento = gump:NewButton(baseframe, nil, "DetailsSegmentButton"..instancia.meu_id, nil, 16, 16, segmentSelector_OnClick, nil, nil, [[Interface\AddOns\DetailsWotlkPort\images\segmentos_icone]])
	...
	reportButton:SetNormalTexture([[Interface\AddOns\DetailsWotlkPort\images\toolbar_icons]])
	reportButton:GetNormalTexture():SetTexCoord(96/256, 128/256, 0, 1)
	reportButton:SetHighlightTexture([[Interface\AddOns\DetailsWotlkPort\images\toolbar_icons]])
	reportButton:GetHighlightTexture():SetTexCoord(96/256, 128/256, 0, 1)
	reportButton:SetPushedTexture([[Interface\AddOns\DetailsWotlkPort\images\toolbar_icons]])
	...
	resetButton:SetNormalTexture([[Interface\AddOns\DetailsWotlkPort\images\toolbar_icons]])
	resetButton:GetNormalTexture():SetTexCoord(128/256, 160/256, 0, 1)

	resetButton:SetHighlightTexture([[Interface\AddOns\DetailsWotlkPort\images\toolbar_icons]])
	resetButton:GetHighlightTexture():SetTexCoord(128/256, 160/256, 0, 1)

	resetButton:SetPushedTexture([[Interface\AddOns\DetailsWotlkPort\images\toolbar_icons]])
