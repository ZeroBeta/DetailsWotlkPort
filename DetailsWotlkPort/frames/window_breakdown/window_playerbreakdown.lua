...
function Details:RegisterFrameToColor(frame)
	...
	elseif frame.SetBackdropColor then
		frame:SetBackdropColor(unpack(colorTable))
	end
end
...
function Details:SetWindowColor(r, g, b, a)
	...
	if breakdownWindowFrame.SetColor then breakdownWindowFrame:SetColor(r, g, b, a) elseif breakdownWindowFrame.SetBackdropColor then breakdownWindowFrame:SetBackdropColor(r, g, b, a) end
	if breakdownSideMenu.SetColor then breakdownSideMenu:SetColor(r, g, b, a) elseif breakdownSideMenu.SetBackdropColor then breakdownSideMenu:SetBackdropColor(r, g, b, a) end

	if (DetailsOptionsWindow) then
		if DetailsOptionsWindow.SetColor then DetailsOptionsWindow:SetColor(r, g, b, a) elseif DetailsOptionsWindow.SetBackdropColor then DetailsOptionsWindow:SetBackdropColor(r, g, b, a) end
		if DetailsPluginContainerWindowMenuFrame.SetColor then DetailsPluginContainerWindowMenuFrame:SetColor(r, g, b, a) elseif DetailsPluginContainerWindowMenuFrame.SetBackdropColor then DetailsPluginContainerWindowMenuFrame:SetBackdropColor(r, g, b, a) end
	end

	if (DetailsReportWindow) then
		if DetailsReportWindow.SetColor then DetailsReportWindow:SetColor(r, g, b, a) elseif DetailsReportWindow.SetBackdropColor then DetailsReportWindow:SetBackdropColor(r, g, b, a) end
	end

	if (DetailsAllAttributesFrame) then
		if DetailsAllAttributesFrame.SetColor then DetailsAllAttributesFrame:SetColor(r, g, b, a) elseif DetailsAllAttributesFrame.SetBackdropColor then DetailsAllAttributesFrame:SetBackdropColor(r, g, b, a) end
	end

	if (DetailsSpellBreakdownOptionsPanel) then
		if DetailsSpellBreakdownOptionsPanel.SetColor then DetailsSpellBreakdownOptionsPanel:SetColor(r, g, b, a) elseif DetailsSpellBreakdownOptionsPanel.SetBackdropColor then DetailsSpellBreakdownOptionsPanel:SetBackdropColor(r, g, b, a) end
	end

	for idx, frame in ipairs(Details222.RegisteredFramesToColor) do
		if frame.SetColor then frame:SetColor(r, g, b, a) elseif frame.SetBackdropColor then frame:SetBackdropColor(r, g, b, a) end
	end
		...
		if (instance:IsEnabled()) then
			...
			if (fullWindowFrame.__rcorners) then
				if (fullWindowFrame.BottomHorizontalEdge:IsShown()) then
					if fullWindowFrame.SetColor then fullWindowFrame:SetColor(r, g, b, a) elseif fullWindowFrame.SetBackdropColor then fullWindowFrame:SetBackdropColor(r, g, b, a) end
				end
			end
		end
	end
...
function Details:OpenBreakdownWindow(instanceObject, actorObject, bFromAttributeChange, bIsRefresh, bIsShiftKeyDown, bIsControlKeyDown, bIgnoreOverrides, mainAttributeOverride, subAttributeOverride)
	...
	breakdownWindowFrame.classIcon:SetTexture("Interface\\AddOns\\DetailsWotlkPort\\images\\classes") --top left
...
function breakdownWindowFrame.SetClassIcon(actorObject, class)
	...
	elseif (actorObject.spec and _detalhes.class_specs_coords[actorObject.spec]) then
		breakdownWindowFrame.classIcon:SetTexture([[Interface\AddOns\DetailsWotlkPort\images\spec_icons_normal_alpha]])
		...
		if (coords) then
			...
			breakdownWindowFrame.classIcon:SetTexture("Interface\\AddOns\\DetailsWotlkPort\\images\\classes")
