...
function Details:InstanceCall(func, ...)
	...
	local instances = Details.tabela_instancias or {}
...
function Details:InstanceGroupCall(instance, funcName, ...)
	local group = instance and instance.GetInstanceGroup and instance:GetInstanceGroup()
	if not group then
		return
	end
	for _, thisInstance in ipairs(group) do
		if thisInstance[funcName] then
			...
			for index, iid in pairs(esta_instancia.snap) do -- index = 1 left , 3 right, 2 bottom, 4 top
				if (iid and (iid == ID or id == ID)) then -- iid = instancia.meu_id
					...
					if (index == 2) then  -- index � o codigo do snap
						...
						--esta_instancia.baseframe.rodape.esquerdo:SetTexture("Interface\\AddOns\\DetailsWotlkPort\\images\\bar_down_left")
		...
		if (esta_instancia:IsAtiva()) then
			if (esta_instancia.baseframe.rodape.esquerdo.have_snap) then
				if (remove) then
					...
					--esta_instancia.baseframe.rodape.esquerdo:SetTexture("Interface\\AddOns\\DetailsWotlkPort\\images\\bar_down_left_snap")
				end
			end
		end
	end
...
--cria uma janela para uma nova inst�ncia
	...
	--must create a new one
		...
		new_instance.wallpaper.texture = "Interface\\AddOns\\DetailsWotlkPort\\images\\background"
	...
	function Details:CreateNewInstance(instanceId)
		...
		--setup default wallpaper
		newInstance.wallpaper.texture = "Interface\\AddOns\\DetailsWotlkPort\\images\\background"
...
function Details:PostponeSwitchToCurrent(instance)
	if (
		...
		if (instance.segmento > 0 and instance.auto_current) then
			...
			instance:InstanceAlert(Loc ["STRING_CHANGED_TO_CURRENT"], {[[Interface\AddOns\DetailsWotlkPort\images\toolbar_icons]], 18, 18, false, 32/256, 64/256, 0, 1}, 6)
			return
		else
			return
		end
...
local menu_icones = {
	"Interface\\AddOns\\DetailsWotlkPort\\images\\atributos_icones_damage",
	"Interface\\AddOns\\DetailsWotlkPort\\images\\atributos_icones_heal",
	"Interface\\AddOns\\DetailsWotlkPort\\images\\atributos_icones_energyze",
	"Interface\\AddOns\\DetailsWotlkPort\\images\\atributos_icones_misc"
}
...
function Details:MontaAtributosOption (instancia, func)
	...
	for i = 1, atributos[0] do --[0] armazena quantos atributos existem
		...
		CoolTip:AddIcon ("Interface\\AddOns\\DetailsWotlkPort\\images\\atributos_icones", 1, 1, 20, 20, p*(i-1), p*(i), 0, 1)
	...
	CoolTip:AddIcon ("Interface\\AddOns\\DetailsWotlkPort\\images\\atributos_icones", 1, 1, 20, 20, p*(5-1), p*(5), 0, 1)
	...
	CoolTip:SetOption("StatusBarTexture", [[Interface\AddOns\DetailsWotlkPort\images\bar4_vidro]])
