...
local class_type = Details.atributos.misc
...
function Details.SortIfHaveKey(table1, table2)
...
function Details.SortGroupIfHaveKey(table1, table2)
...
function Details.SortGroupMisc(container, keyName2)
	keyName = keyName2
	return table.sort(container, Details.SortKeyGroupMisc)
end

function Details.SortKeyGroupMisc(table1, table2)
	if (table1.grupo and table2.grupo) then
		return table1[keyName] > table2[keyName]
...
function Details.SortKeySimpleMisc(table1, table2)
	return table1[keyName] > table2[keyName]
end

function Details:ContainerSortMisc(container, amount, keyName2)
	keyName = keyName2
	table.sort(container, Details.SortKeySimpleMisc)
...
local statusBarBackgroundTable_ForDeathTooltip = {
	value = 100,
	texture = [[Interface\AddOns\DetailsWotlkPort\images\bar_serenity]],
...
function Details.ShowDeathTooltip(instance, lineFrame, combatObject, deathTable) --~death
	...
	gameCooltip:AddIcon("Interface\\AddOns\\DetailsWotlkPort\\images\\small_icons", 1, 1, nil, nil, .75, 1, 0, 1)
...
function atributo_misc:ToolTipCC(instancia, numero, barra)
	...
	for _spellid, _tabela in pairs(habilidades) do
		...
		for target_name, amount in pairs(_tabela.targets) do
			...
			GameCooltip:AddIcon([[Interface\AddOns\DetailsWotlkPort\images\espadas]], nil, 1, lineHeight, lineHeight)
			if (classe) then
				GameCooltip:AddIcon([[Interface\AddOns\DetailsWotlkPort\images\classes_small]], nil, 2, lineHeight, lineHeight, unpack(Details.class_coords [classe]))
			else
	...
	for i = 1, min(25, #alvos_dispelados) do
		...
		if (targetActor) then
			...
			if (classe == "UNKNOW") then
				...
				GameCooltip:AddIcon("Interface\\AddOns\\DetailsWotlkPort\\images\\classes_small", nil, nil, 14, 14, unpack(Details.class_coords [classe]))
			end
		end
	end
	...
	if (#meus_pets > 0) then --teve ajudantes
		...
		for index, _table in ipairs(totais) do
			...
			if (_table [2] > 0 and (index < 3 or ismaximized)) then
				...
				Details:AddTooltipBackgroundStatusbar()
				GameCooltip:AddIcon([[Interface\AddOns\DetailsWotlkPort\images\classes_small]], 1, 1, 14, 14, 0.25, 0.49609375, 0.75, 1)
			end
		end
