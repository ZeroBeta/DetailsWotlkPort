...
--constants
	...
	--templates
	local byspell_tooltip_background = {value = 100, color = {0.1960, 0.1960, 0.1960, 0.9097}, texture = [[Interface\AddOns\DetailsWotlkPort\images\bar_background_dark]]}
	local enemies_background = {value = 100, color = {0.1960, 0.1960, 0.1960, 0.8697}, texture = "Interface\\AddOns\\DetailsWotlkPort\\images\\bar_background2"}
...
--damage taken by spell
	...
	function Details:ToolTipBySpell(instance, tabela, thisLine, keydown)
		...
		GameCooltip:SetOption("StatusBarTexture", "Interface\\AddOns\\DetailsWotlkPort\\images\\bar_serenity")
		...
		--GameCooltip:AddIcon([[Interface\AddOns\DetailsWotlkPort\images\key_shift]], 1, 2, Details.tooltip_key_size_width, Details.tooltip_key_size_height, 0, 1, 0, 0.640625, Details.tooltip_key_overlay2)
		...
		for index, t in ipairs(Targets) do
			...
			if (class) then
				...
				if (specID) then
					...
					GameCooltip:AddIcon("Interface\\AddOns\\DetailsWotlkPort\\images\\classes_small_alpha", 1, 1,iconSize,iconSize, l, r, t, b)
				end

			elseif(t[1] == Loc ["STRING_TARGETS_OTHER1"]) then
				GameCooltip:AddIcon("Interface\\AddOns\\DetailsWotlkPort\\images\\classes_small_alpha", 1, 1,iconSize,iconSize, 0.25, 0.49609375, 0.75, 1)
			end
		end
...
--frags
				...
				for i = 1, math.min(min, #damage_taken_table) do
					...
					if (classe == "UNKNOW") then
						...
						if (specID) then
							...
							GameCooltip:AddIcon([[Interface\AddOns\DetailsWotlkPort\images\classes_small_alpha]], nil, nil, iconSize, iconSize, unpack(Details.class_coords [classe]))
						end
					end
	...
	function damageClass:AtualizarFrags(tabela, whichRowLine, colocacao, instancia)
		...
		if (tabela [3] == "UNKNOW" or tabela [3] == "UNGROUPPLAYER" or tabela [3] == "ENEMY") then
			thisLine.icone_classe:SetTexture([[Interface\AddOns\DetailsWotlkPort\images\classes_plus]])
		...
		if (arenaEnemy) then
			...
			if (instance.row_info.show_faction_icon) then
				...
				if (Details.faction_against == "Horde") then
					local leftText = barNumber .. "|TInterface\\AddOns\\DetailsWotlkPort\\images\\icones_barra:" ..(instance.row_info.height + sizeOffset)..":"..(instance.row_info.height + sizeOffset) .. ":0:0:256:32:0:32:0:32|t"..self.displayName
					if (usingCustomLeftText) then
						bar.lineText1:SetText(stringReplace(instance.row_info.textL_custom_text, bar.colocacao, self.displayName, "|TInterface\\AddOns\\DetailsWotlkPort\\images\\icones_barra:" ..(instance.row_info.height + sizeOffset)..":"..(instance.row_info.height + sizeOffset) .. ":0:0:256:32:0:32:0:32|t", self, instance:GetCombat(), instance, leftText))
					else
						bar.lineText1:SetText(leftText) --seta o texto da esqueda -- HORDA
					end
				else --alliance
					local leftText = barNumber .. "|TInterface\\AddOns\\DetailsWotlkPort\\images\\icones_barra:" ..(instance.row_info.height + sizeOffset)..":"..(instance.row_info.height + sizeOffset) .. ":0:0:256:32:32:64:0:32|t"..self.displayName
					if (usingCustomLeftText) then
						bar.lineText1:SetText(stringReplace(instance.row_info.textL_custom_text, bar.colocacao, self.displayName, "|TInterface\\AddOns\\DetailsWotlkPort\\images\\icones_barra:" ..(instance.row_info.height + sizeOffset)..":"..(instance.row_info.height + sizeOffset) .. ":0:0:256:32:32:64:0:32|t", self, instance:GetCombat(), instance, leftText))
					else
			...
			if (prescienceData) then
				...
				for amountOfPrescienceApplied, time in ipairs(prescienceData) do
					if (time > 0) then
						...
						GameCooltip:AddIcon([[Interface\AddOns\DetailsWotlkPort\images\spells\prescience_time]], nil, nil, iconSize, iconSize)
	...
	do
		...
		--MOSTRA HABILIDADES
			...
			if (is_maximized) then
				...
				--GameCooltip:AddIcon([[Interface\AddOns\DetailsWotlkPort\images\key_shift]], 1, 2, Details.tooltip_key_size_width, Details.tooltip_key_size_height, 0, 1, 0, 0.640625, Details.tooltip_key_overlay1)
		...
		--targets(enemies)
			...
			if (instance.sub_atributo == 1 or instance.sub_atributo == 6) then
				...
				Details:AddTooltipSpellHeaderText(Loc ["STRING_TARGETS"], headerColor, #ActorTargetsSortTable, [[Interface\Addons\DetailsWotlkPort\images\icons]], 0, 0.03125, 0.126953125, 0.15625)
				...
				if (is_maximized) then
					--highlight
					GameCooltip:AddIcon([[Interface\AddOns\DetailsWotlkPort\images\key_ctrl]], 1, 2, Details.tooltip_key_size_width, Details.tooltip_key_size_height, 0, 1, 0, 0.640625, Details.tooltip_key_overlay2)
					Details:AddTooltipHeaderStatusbar(r, g, b, 1)
				else
					GameCooltip:AddIcon([[Interface\AddOns\DetailsWotlkPort\images\key_ctrl]], 1, 2, Details.tooltip_key_size_width, Details.tooltip_key_size_height, 0, 1, 0, 0.640625, Details.tooltip_key_overlay1)
					Details:AddTooltipHeaderStatusbar(r, g, b, barAlha)
				end
		...
		for index, damageTable in ipairs(petDamageTable) do
			if (damageTable [2] > 0 and(index <= Details.tooltip.tooltip_max_pets or ismaximized)) then
				if (not petHeaderAdded) then
					...
					if (ismaximized) then
						GameCooltip:AddIcon([[Interface\AddOns\DetailsWotlkPort\images\key_alt]], 1, 2, Details.tooltip_key_size_width, Details.tooltip_key_size_height, 0, 1, 0, 0.640625, Details.tooltip_key_overlay2)
						Details:AddTooltipHeaderStatusbar(r, g, b, 1)
					else
						GameCooltip:AddIcon([[Interface\AddOns\DetailsWotlkPort\images\key_alt]], 1, 2, Details.tooltip_key_size_width, Details.tooltip_key_size_height, 0, 1, 0, 0.640625, Details.tooltip_key_overlay1)
						Details:AddTooltipHeaderStatusbar(r, g, b, barAlha)
					end
				end
				...
				GameCooltip:AddIcon([[Interface\AddOns\DetailsWotlkPort\images\classes_small_alpha]], 1, 1, icon_size.W, icon_size.H, 0.25/2, 0.49609375/2, 0.75/2, 1/2)
			end
		end
	end
...
function damageClass:ToolTip_DamageTaken(instance, numero, barra, keydown)
	...
	if (subAttribute == DETAILS_SUBATTRIBUTE_ENEMIES) then
		...
		--Details:AddTooltipSpellHeaderText(Loc ["STRING_FROM"], headerColor, #damageTakenDataSorted, [[Interface\Addons\DetailsWotlkPort\images\icons]], 0.126953125, 0.1796875, 0, 0.0546875)
	end
...
function damageClass:ToolTip_FriendlyFire(instancia, numero, barra, keydown)
	...
	for i = 1, math.min(max_abilities, #DamagedPlayers) do
		...
		GameCooltip:AddIcon("Interface\\AddOns\\DetailsWotlkPort\\images\\espadas", nil, nil, lineHeight, lineHeight)
		Details:AddTooltipBackgroundStatusbar()

		if (classe == "UNKNOW") then
			GameCooltip:AddIcon("Interface\\AddOns\\DetailsWotlkPort\\images\\classes_small", nil, nil, lineHeight, lineHeight, unpack(Details.class_coords ["UNKNOW"]))
		else
			...
			if (specID) then
				...
				GameCooltip:AddIcon("Interface\\AddOns\\DetailsWotlkPort\\images\\classes_small", nil, nil, lineHeight, lineHeight, unpack(Details.class_coords [classe]))
			end
		end
...
--[[exported]] function Details:UpdadeInfoBar(row, index, spellId, name, value, formattedValue, max, percent, icon, detalhes, texCoords, spellSchool, class)
	...
	if (icon) then
		...
		if (icon == "Interface\\AddOns\\DetailsWotlkPort\\images\\classes_small") then
...
function damageClass:MontaInfoDamageDone()
		...
		if (petActor) then --PET
			...
			if (bShouldMergeSpellsWithThePet and petSpellContainer:HasTwoOrMoreSpells()) then
				...
				local bkSpellData = {
					...
					bCanExpand = true,
					actorIcon = [[Interface\AddOns\DetailsWotlkPort\images\pets\pet_icon_1]],
				}
				detailsFramework:Mixin(bkSpellData, Details.SpellTableMixin)
	...
	if (instance.sub_atributo == DETAILS_SUBATTRIBUTE_ENEMIES) then
		...
		for index, thisEnemyTable in ipairs(enemyTable) do
			...
			thisLine.icone:SetTexture([[Interface\AddOns\DetailsWotlkPort\images\classes_small_alpha]]) --class icon
		...
		for index, targetTable in ipairs(allActorTargets) do
			...
			if (targetActorObject) then
				...
				barra.icone:SetTexture([[Interface\AddOns\DetailsWotlkPort\images\classes_small_alpha]]) --CLASSE
...
function damageClass:MontaDetalhesEnemy(spellid, barra)
	...
	for index, tabela in ipairs(target_pool) do
		...
		barra.icone:SetTexture("Interface\\AddOns\\DetailsWotlkPort\\images\\classes_small_alpha")
		barra.icone:SetTexCoord(unpack(texCoords))
...
--[[
	...
	if (enemy) then
		...
		GameCooltip:AddIcon([=[Interface\AddOns\DetailsWotlkPort\images\key_shift]=], 2, 2, Details.tooltip_key_size_width, Details.tooltip_key_size_height, 0, 1, 0, 0.640625, Details.tooltip_key_overlay2)
