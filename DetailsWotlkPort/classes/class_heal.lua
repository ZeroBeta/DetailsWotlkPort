...
healingClass.__index = healingClass
...
local function SafeGetSpellInfo(spellId, fallbackName)
	local name, _, icon = _GetSpellInfo(spellId)
	if (not name or name == "") then
		name = fallbackName or ((spellId == 0 or spellId == nil) and "Absorb" or "Unknown")
	end
	if (not icon) then
...
--local npchealingtaken_tooltip_background = {value = 100, color = {0.1960, 0.1960, 0.1960, 0.9097}, texture = [[Interface\AddOns\DetailsWotlkPort\images\bar_background2]]}
...
function healingClass:ToolTip_HealingDenied (instancia, numero, barra, keydown)
	...
	--Spells
		...
		if (keydown == "shift" or TooltipMaximizedMethod == 2 or TooltipMaximizedMethod == 3) then
			--GameCooltip:AddIcon ([[Interface\AddOns\DetailsWotlkPort\images\key_shift]], 1, 2, _detalhes.tooltip_key_size_width, _detalhes.tooltip_key_size_height, 0, 1, 0, 0.640625, _detalhes.tooltip_key_overlay2)
			--_detalhes:AddTooltipHeaderStatusbar (r, g, b, 1)
			ismaximized = true
		else
			--GameCooltip:AddIcon ([[Interface\AddOns\DetailsWotlkPort\images\key_shift]], 1, 2, _detalhes.tooltip_key_size_width, _detalhes.tooltip_key_size_height, 0, 1, 0, 0.640625, _detalhes.tooltip_key_overlay1)
			--_detalhes:AddTooltipHeaderStatusbar (r, g, b, barAlha)
		end
		...
		for i = 1, _math_min(tooltip_max_abilities, #spellList) do
			...
			local spellName, spellIcon = SafeGetSpellInfo(spellObject.id)
	...
	--Target Players
		...
		if (keydown == "ctrl" or TooltipMaximizedMethod == 2 or TooltipMaximizedMethod == 4) then
			GameCooltip:AddIcon ([[Interface\AddOns\DetailsWotlkPort\images\key_ctrl]], 1, 2, _detalhes.tooltip_key_size_width, _detalhes.tooltip_key_size_height, 0, 1, 0, 0.640625, _detalhes.tooltip_key_overlay2)
			_detalhes:AddTooltipHeaderStatusbar (r, g, b, 1)
			ismaximized = true
		else
			GameCooltip:AddIcon ([[Interface\AddOns\DetailsWotlkPort\images\key_ctrl]], 1, 2, _detalhes.tooltip_key_size_width, _detalhes.tooltip_key_size_height, 0, 1, 0, 0.640625, _detalhes.tooltip_key_overlay1)
			_detalhes:AddTooltipHeaderStatusbar (r, g, b, barAlha)
		end
		...
		for i = 1, _math_min(tooltip_max_abilities2, #playerSorted) do
			...
			if (targetActor) then
				...
				if (classe == "UNKNOW") then
					...
					GameCooltip:AddIcon ("Interface\\AddOns\\DetailsWotlkPort\\images\\classes_small", nil, nil, 14, 14, _unpack(_detalhes.class_coords [classe]))
				end
			end
	...
	for i = 1, _math_min(max, #meus_curadores) do
		...
		if (classe == "UNKNOW") then
			...
			if (specID) then
				...
				GameCooltip:AddIcon ("Interface\\AddOns\\DetailsWotlkPort\\images\\classes_small", nil, nil, lineHeight, lineHeight, _unpack(_detalhes.class_coords [classe]))
			end
		end
...
local background_heal_vs_absorbs = {value = 100, color = {1, 1, 0, .25}, specialSpark = false, texture = [[Interface\AddOns\DetailsWotlkPort\images\bar4_glass]]}
...
function healingClass:ToolTip_HealingDone (instance, numero, barra, keydown)
	...
	for _spellid, _skill in pairs(ActorSkillsContainer) do
		--prefer the recorded spell name even when spellId is zero (classic absorbs often come without an id)
		local fallbackName = _skill.spellName or _skill.nome or ((_spellid == 0) and "Absorb" or tostring(_spellid))
		local SkillName, SkillIcon = SafeGetSpellInfo(_spellid, fallbackName)
		local amount = _skill[skill_key] or 0
		if (amount > 0 or _skill.anti_heal) then
			tinsert(ActorHealingTable, {
				_spellid,
				amount,
				amount/ActorTotal*100,
	...
	for i = 1, _math_min(tooltip_max_abilities, #ActorHealingTable) do
		...
		local spellName = ActorHealingTable[i][4] and ActorHealingTable[i][4][1]
		if (not spellName) then
			spellName = _GetSpellInfo(ActorHealingTable[i][1]) or "Unknown"
		end
		...
		for i = 1, _math_min(tooltip_max_abilities2, #ActorHealingTargets) do
			...
			if (targetActor) then
				...
				if (classe == "UNKNOW") then
					...
					GameCooltip:AddIcon ("Interface\\AddOns\\DetailsWotlkPort\\images\\classes_small", nil, nil, icon_size.W, icon_size.H, _unpack(_detalhes.class_coords [classe]))
				end
			end
		end
	end
		...
		for index, _table in ipairs(totais) do
			...
			if (_table [2] >= 1 and (index < 3 or ismaximized)) then
				...
				if (not added_logo) then
					...
					if (ismaximized) then
						GameCooltip:AddIcon ([[Interface\AddOns\DetailsWotlkPort\images\key_alt]], 1, 2, _detalhes.tooltip_key_size_width, _detalhes.tooltip_key_size_height, 0, 1, 0, 0.640625, _detalhes.tooltip_key_overlay2)
						_detalhes:AddTooltipHeaderStatusbar (r, g, b, 1)
					else
						GameCooltip:AddIcon ([[Interface\AddOns\DetailsWotlkPort\images\key_alt]], 1, 2, _detalhes.tooltip_key_size_width, _detalhes.tooltip_key_size_height, 0, 1, 0, 0.640625, _detalhes.tooltip_key_overlay1)
						_detalhes:AddTooltipHeaderStatusbar (r, g, b, barAlha)
					end
				...
				_detalhes:AddTooltipBackgroundStatusbar()
				GameCooltip:AddIcon ([[Interface\AddOns\DetailsWotlkPort\images\classes_small]], 1, 1, icon_size.W, icon_size.H, 0.25, 0.49609375, 0.75, 1)
			end
		end
	end
...
function healingClass:MontaInfoHealTaken()
	...
	for index, tabela in ipairs(meus_curandeiros) do
		...
		self:UpdadeInfoBar(barra, index, tabela[1], tabela[1], tabela[2], formated_value, max_, tabela[3], "Interface\\AddOns\\DetailsWotlkPort\\images\\classes_small", true, texCoords)
	end
...
--pegar as habilidade de dar sort no heal
	...
	for index, tabela in ipairs(jogadores_overhealed) do
		...
		-- icon
		barra.icone:SetTexture([[Interface\AddOns\DetailsWotlkPort\images\classes_small]])
...
function healingClass:MontaDetalhesHealingTaken (nome, barra)
	...
	for spellid, tabela in pairs(conteudo) do --da foreach em cada spellid do container
		if (tabela.targets [actor]) then
			local fallbackName = tabela.spellName or tabela.nome or ((spellid == 0) and "Absorb" or tostring(spellid))
			local spell_nome, icone = SafeGetSpellInfo(spellid, fallbackName)
...
function healingClass:MontaDetalhesHealingDone (spellid, barra) --deprecated with the new breakdown window
	...
	if (not spellName or spellName == "") then
		spellName, icone = SafeGetSpellInfo(spellid)
	end
