...
function _detalhes:LoadInstanceConfig()
	...
	--ensure row textures point to valid files (prevents black bars if a saved texture isn't available)
	local rowInfo = self.row_info
	if (rowInfo) then
		local defaultTextureFile = SharedMedia:Fetch("statusbar", "Armory") or SharedMedia:Fetch("statusbar", "Details D'ictum") or [[Interface\AddOns\DetailsWotlkPort\images\bar4]]
		local defaultTextureName = "Armory"
		if (rowInfo.texture == "You Are the Best!") then
			rowInfo.texture = defaultTextureName
			rowInfo.texture_file = defaultTextureFile
		end

		local fetchedTexture = rowInfo.texture and SharedMedia:Fetch("statusbar", rowInfo.texture)
		if (not fetchedTexture) then
			rowInfo.texture = defaultTextureName
			rowInfo.texture_file = defaultTextureFile
		else
			rowInfo.texture_file = fetchedTexture
		end

		if (not rowInfo.texture_background or rowInfo.texture_background == "" or not SharedMedia:Fetch("statusbar", rowInfo.texture_background)) then
			rowInfo.texture_background = defaultTextureName
			rowInfo.texture_background_file = defaultTextureFile
		elseif (not rowInfo.texture_background_file or rowInfo.texture_background_file == "") then
			rowInfo.texture_background_file = SharedMedia:Fetch("statusbar", rowInfo.texture_background) or defaultTextureFile
		end
	end
end
...
_detalhes.instance_defaults = {
	...
	--left to right or right to left bars
		bars_inverted = false,
	--toolbar icons file
		--toolbar_icon_file = [[Interface\AddOns\DetailsWotlkPort\images\toolbar_icons_grayscale]],
	...
	--row info
		...
		--bar texture (default to Armory to avoid missing texture black bars)
			texture = "Armory",
			texture_custom = "",
		--bar texture name
			texture_file = SharedMedia:Fetch("statusbar", "Armory") or [[Interface\AddOns\DetailsWotlkPort\images\bar4]],
