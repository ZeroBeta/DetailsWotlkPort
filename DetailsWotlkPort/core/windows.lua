...
--cooltip presets
	local preset3_backdrop = {bgFile = [[Interface\DialogFrame\UI-DialogBox-Background-Dark]], edgeFile = [[Interface\AddOns\DetailsWotlkPort\images\border_3]], tile=true,
	...
	local preset2_backdrop = {bgFile = [[Interface\AddOns\DetailsWotlkPort\images\background]], edgeFile = [[Interface\Buttons\WHITE8X8]], tile=true,
...
--feedback window
	function Details:OpenFeedbackWindow()
		if (not _G.DetailsFeedbackPanel) then
			...
			local wowi = gump:NewImage(panel, [[Interface\AddOns\DetailsWotlkPort\images\icons2]], 101, 34, "artwork", {0/512, 101/512, 163/512, 200/512})
			local curse = gump:NewImage(panel, [[Interface\AddOns\DetailsWotlkPort\images\icons2]], 101, 34, "artwork", {0/512, 101/512, 201/512, 242/512})
			local mmoc = gump:NewImage(panel, [[Interface\AddOns\DetailsWotlkPort\images\icons2]], 101, 34, "artwork", {0/512, 101/512, 243/512, 285/512})
	...
	f.logo:SetTexture([[Interface\AddOns\DetailsWotlkPort\images\logotipo]])
...
--update details version window
	function Details:OpenUpdateWindow()
		...
		if (not _G.DetailsUpdateDialog) then
			...
			updatewindow_frame.gnoma:SetTexture("Interface\\AddOns\\DetailsWotlkPort\\images\\icons2")
	...
	function Details:RegisterMinimap()
		...
		Details.minimap = Details.minimap or {hide = false}

		if LDB then
			local databroker = LDB:NewDataObject ("Details", {
				type = "data source",
				icon = [[Interface\AddOns\DetailsWotlkPort\images\minimap]] or [[Interface\Icons\INV_Misc_QuestionMark]],
				text = "0",
				...
				OnClick = function(self, button)
					...
					elseif (button == "RightButton") then
						...
						GameCooltip:SetOption("TextSize", 10)
						-- ensure icons use cropped texcoords and blend properly on Wrath
						GameCooltip:SetOption("IconTexCoord", {0.07, 0.93, 0.07, 0.93})
						...
						GameCooltip:AddIcon ([[Interface\COMMON\VOICECHAT-MUTED]], 1, 1, 14, 14, 0.15, 0.85, 0.15, 0.85)

						GameCooltip:AddLine("$div")

						--nova instancia
						GameCooltip:AddMenu (1, Details.CriarInstancia, true, nil, nil, Loc ["STRING_MINIMAPMENU_NEWWINDOW"], nil, true)
						GameCooltip:AddIcon ([[Interface\AddOns\DetailsWotlkPort\images\icons]], 1, 1, 12, 11, 462/512, 473/512, 1/512, 11/512)
						...
						GameCooltip:AddIcon ([[Interface\Buttons\UI-Panel-HideButton-Disabled]], 1, 1, 14, 14, 7/32, 24/32, 8/32, 24/32)
			...
			-- ensure visibility honors saved setting
			if LDBIcon and LDBIcon.IsRegistered and LDBIcon:IsRegistered("Details") then
				if self.minimap.hide then
					LDBIcon:Hide("Details")
				else
					LDBIcon:Show("Details")
				end
	...
	function Details:DoRegisterHotCorner()
		...
		if (_G.HotCorners) then
			_G.HotCorners:RegisterHotCornerButton (
				...
				--icon
				[[Interface\AddOns\DetailsWotlkPort\images\minimap]],
				--tooltip
				tooltip_hotcorner,
				--click function
...
function Details:InitializeAPIWindow()
	...
	DetailsAPI2Frame.__icon = [[Interface\AddOns\DetailsWotlkPort\images\icons]]
...
function Details:FormatBackground(frame) --deprecated I guess
	frame:SetBackdrop({edgeFile = [[Interface\Buttons\WHITE8X8]], edgeSize = 1, bgFile = [[Interface\AddOns\DetailsWotlkPort\images\background]], tileSize = 64, tile = true})
	...
	frame.__background:SetTexture([[Interface\AddOns\DetailsWotlkPort\images\background]], true)
