...
--send functions
	...
	function Details:SendPluginCommMessage(prefix, channel, ...)
		local payload = Details:Serialize(self.__version, ...)

		if (channel == "RAID") then
			Details:SendCommMessage(prefix, payload, GetGroupChannel(true))
		elseif (channel == "PARTY") then
			Details:SendCommMessage(prefix, payload, GetGroupChannel(false))
		else
			Details:SendCommMessage(prefix, payload, channel)
		end
	...
	function Details:SendRaidData(type, ...)
		local channel = GetGroupChannel(true)
		Details:SendCommMessage(DETAILS_PREFIX_NETWORK, Details:Serialize(type, UnitName("player"), GetRealmName(), Details.realversion, ...), channel)
	end
	...
	function Details:SendPartyData(type, ...)
		...
		local channel = GetGroupChannel(false)
