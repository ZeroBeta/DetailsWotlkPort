...
--local pointers
	...
	local function EnsureDefensiveCooldowns()
		if (next(defensive_cooldowns)) then
			return
		end
	...
	--spell reflection
		...
		--classic absorb tracking (3.3.5 style combat log)
		local classicAbsorb = {
			activeShields = Details.ActiveShields or {},
			recentHeals = Details.RecentHeals or {},
			spellNameOverride = {},
		}
		Details.ActiveShields = classicAbsorb.activeShields
		Details.RecentHeals = classicAbsorb.recentHeals

		--priority tables borrowed from the Skada absorb tracker
		classicAbsorb.data = {
			mage_ward = {
			...
			spellData = {
				...
				[17] = {dur = 30, cap = 10000}, [592] = {dur = 30, cap = 10000}, [600] = {dur = 30, cap = 10000}, [3747] = {dur = 30, cap = 10000},
				[6065] = {dur = 30, cap = 10000}, [6066] = {dur = 30, cap = 10000}, [10898] = {dur = 30, cap = 10000}, [10899] = {dur = 30, cap = 10000},
				[10900] = {dur = 30, cap = 10000}, [10901] = {dur = 30, cap = 10000}, [10902] = {dur = 30, cap = 10000}, [10903] = {dur = 30, cap = 10000},
				[10904] = {dur = 30, cap = 10000}, [10905] = {dur = 30, cap = 10000}, [25217] = {dur = 30, cap = 10000}, [25218] = {dur = 30, cap = 10000},
				[25315] = {dur = 30, cap = 10000}, [48065] = {dur = 30, cap = 10000}, [48066] = {dur = 30, cap = 10000}, [1110901] = {dur = 30, cap = 10000},

				[58597] = {dur = 30, cap = 8000},
				[28527] = {dur = 10, cap = 8000},
				[65684] = {dur = 15},
				[65686] = {dur = 15},
				[70845] = {dur = 6},
			},
		...
		classicAbsorb.spellLookup = {}
		function classicAbsorb.RegisterAbsorbSpell(spellId, info)
			classicAbsorb.spellLookup[spellId] = true
			if (info and not classicAbsorb.data.spellData[spellId]) then
				classicAbsorb.data.spellData[spellId] = info
			end
		end

		for spellId in pairs(classicAbsorb.data.spellData) do
			classicAbsorb.spellLookup[spellId] = true
		end
		for spellId in pairs(classicAbsorb.data.mage_ward) do
			classicAbsorb.RegisterAbsorbSpell(spellId, {dur = 30})
		end
		for spellId in pairs(classicAbsorb.data.warlock_shadow_ward) do
			classicAbsorb.RegisterAbsorbSpell(spellId, {dur = 30})
		end
		for spellId in pairs(classicAbsorb.data.mage_ice_barrier) do
			classicAbsorb.RegisterAbsorbSpell(spellId, {dur = 60})
		end
		for spellId in pairs(classicAbsorb.data.warlock_sacrifice) do
			classicAbsorb.RegisterAbsorbSpell(spellId, {dur = 30})
		end
		for spellId in pairs(classicAbsorb.data.priest_divine_aegis) do
			classicAbsorb.RegisterAbsorbSpell(spellId, classicAbsorb.data.spellData[spellId] or {dur = 12, stackable = true})
		end

		function classicAbsorb.GetShieldTargetKey(targetSerial, targetName)
			if (targetSerial and targetSerial ~= "") then
				return targetSerial
			end
			return targetName
		end
		function classicAbsorb.SortShieldsByPriority(a, b)
			local a_spellid = a and a.spellid
			local b_spellid = b and b.spellid
			if (not a_spellid or not b_spellid) then
				return false
			end
			...
			if (classicAbsorb.data.warlock_shadow_ward[a_spellid] or classicAbsorb.data.mage_ward[a_spellid]) then return false end
			if (classicAbsorb.data.warlock_shadow_ward[b_spellid] or classicAbsorb.data.mage_ward[b_spellid]) then return true end
			...
			if (classicAbsorb.data.priest_divine_aegis[a_spellid]) then return false end
			if (classicAbsorb.data.priest_divine_aegis[b_spellid]) then return true end

			if (classicAbsorb.data.mage_ice_barrier[a_spellid]) then return false end
			if (classicAbsorb.data.mage_ice_barrier[b_spellid]) then return true end

			if (classicAbsorb.data.warlock_sacrifice[a_spellid]) then return false end
			if (classicAbsorb.data.warlock_sacrifice[b_spellid]) then return true end

			if (a_spellid == 881472) then return false end
			if (b_spellid == 881472) then return true end
			if (a_spellid == 84380) then return false end
			if (b_spellid == 84380) then return true end

			return (a.ts or 0) > (b.ts or 0)
		end

		function classicAbsorb.PruneShields(targetKey, now)
			local shieldList = classicAbsorb.activeShields[targetKey]
			if (not shieldList) then
				return
			end
			for i = #shieldList, 1, -1 do
				local entry = shieldList[i]
				if (entry.expires and entry.expires < now) then
					table.remove(shieldList, i)
				end
			end
			if (#shieldList == 0) then
				classicAbsorb.activeShields[targetKey] = nil
			end
		end

		function classicAbsorb.StoreRecentHeal(timestamp, sourceName, targetName, amount)
			if (not sourceName or not targetName or not amount) then
				return
			end
			classicAbsorb.recentHeals[targetName] = classicAbsorb.recentHeals[targetName] or {}
			classicAbsorb.recentHeals[targetName][sourceName] = {ts = timestamp, amount = amount}
		end

		function classicAbsorb.RegisterShield(timestamp, sourceSerial, sourceName, sourceFlags, targetSerial, targetName, spellId, spellSchool, amount, spellName)
			if (not targetName) then
				return
			end

			if (spellName) then
				local pwsName = _GetSpellInfo(17)
				if (pwsName and spellName == pwsName) then
					spellId = 17
					classicAbsorb.spellNameOverride[17] = classicAbsorb.spellNameOverride[17] or "Power Word: Shield"
				end
			end

			if (not classicAbsorb.spellLookup[spellId]) then
				return
			end

			local data = classicAbsorb.data.spellData[spellId] or {}
			local duration = data.dur or 15
			local targetKey = classicAbsorb.GetShieldTargetKey(targetSerial, targetName)
			if (not targetKey) then
				return
			end

			classicAbsorb.PruneShields(targetKey, timestamp)

			local calculatedAmount = amount or data.cap
			if (data.calc == "divine_aegis") then
				local healInfo = classicAbsorb.recentHeals[targetName] and classicAbsorb.recentHeals[targetName][sourceName]
				if (healInfo and (timestamp - healInfo.ts) < 0.3) then
					calculatedAmount = floor((calculatedAmount or 0) + (healInfo.amount or 0) * 0.3)
				end
			elseif (data.calc == "valannyr") then
				local healInfo = classicAbsorb.recentHeals[targetName] and classicAbsorb.recentHeals[targetName][sourceName]
				if (healInfo and (timestamp - healInfo.ts) < 0.3) then
					calculatedAmount = floor((calculatedAmount or 0) + min((data.cap or 20000), (healInfo.amount or 0) * 0.15))
				end
			elseif (data.calc == "ams") then
				local maxHealth = UnitHealthMax and UnitHealthMax(targetName)
				if (maxHealth) then
					calculatedAmount = floor((calculatedAmount or 0) + maxHealth * 0.75)
				end
			end

			local shieldEntry = {
				srcGUID = sourceSerial,
				srcName = sourceName,
			...
			if (not shieldList) then
				shieldList = {}
				classicAbsorb.activeShields[targetKey] = shieldList
			end

			if (data.stackable) then
				for _, entry in ipairs(shieldList) do
					if (entry.spellid == spellId and entry.srcName == sourceName) then
						entry.amount = (entry.amount or 0) + (calculatedAmount or 0)
						entry.expires = timestamp + duration
						entry.ts = timestamp
						return
					end
				end
			else
				for idx = #shieldList, 1, -1 do
					local entry = shieldList[idx]
					if (entry.spellid == spellId and entry.srcName == sourceName) then
						table.remove(shieldList, idx)
					end
				end
			end

			shieldList[#shieldList+1] = shieldEntry
		end

		function classicAbsorb.RemoveShield(timestamp, targetSerial, targetName, spellId, sourceName)
			local targetKey = classicAbsorb.GetShieldTargetKey(targetSerial, targetName)
			if (not targetKey) then
				return
			end
			...
			for i = #shieldList, 1, -1 do
				local entry = shieldList[i]
				if (entry.spellid == spellId and (not sourceName or entry.srcName == sourceName)) then
					table.remove(shieldList, i)
				end
			end
		...
		function classicAbsorb.ApplyAbsorb(timestamp, targetSerial, targetName, targetFlags, targetFlags2, absorbed, school)
			if (not targetName or not absorbed or absorbed <= 0) then
				return
			end
			...
			classicAbsorb.PruneShields(targetKey, timestamp)
			local shieldList = classicAbsorb.activeShields[targetKey]
			if (not shieldList or #shieldList == 0) then
				return
			end

			table.sort(shieldList, classicAbsorb.SortShieldsByPriority)
			...
			for i = #shieldList, 1, -1 do
				...
				if (entry.amount and entry.amount > 0) then
					credit = min(entry.amount, remaining)
					entry.amount = entry.amount - credit
				end

				local displayName = classicAbsorb.spellNameOverride[entry.spellid] or _GetSpellInfo(entry.spellid) or "Absorb"
				parser:heal("SPELL_HEAL", timestamp, entry.srcGUID, entry.srcName, entry.srcFlags, targetSerial, targetName, targetFlags, targetFlags2, entry.spellid, displayName, entry.school or school or 1, credit, 0, 0, nil, true)
			...
			if (remaining > 0 and lastShield) then
				local displayName = classicAbsorb.spellNameOverride[lastShield.spellid] or _GetSpellInfo(lastShield.spellid) or "Absorb"
				parser:heal("SPELL_HEAL", timestamp, lastShield.srcGUID, lastShield.srcName, lastShield.srcFlags, targetSerial, targetName, targetFlags, targetFlags2, lastShield.spellid, displayName, lastShield.school or school or 1, remaining, 0, 0, nil, true)
			end

			if (shieldList and #shieldList == 0) then
				classicAbsorb.activeShields[targetKey] = nil
			end

			-- force UI refresh so damage prevented rows update during combat
			_current_heal_container.need_refresh = true
			_current_combat.changed = true
			_current_combat.last_actor = nil
		end

		function classicAbsorb.HandleAura(token, timestamp, sourceSerial, sourceName, sourceFlags, targetSerial, targetName, targetFlags, targetFlags2, spellId, spellName, spellSchool, auraType, amount)
			if (token == "SPELL_AURA_APPLIED" or token == "SPELL_AURA_REFRESH") then
				-- Divine Aegis can show up with realm-specific spellIds; register by name if needed.
				if (spellName and (not classicAbsorb.spellLookup[spellId])) then
					local daName = _GetSpellInfo(47753)
					if (daName and spellName == daName) then
						classicAbsorb.RegisterAbsorbSpell(spellId, {dur = 12, stackable = true, calc = "divine_aegis"})
					end
				end
			end

			if (token == "SPELL_AURA_APPLIED" or token == "SPELL_AURA_REFRESH") then
				classicAbsorb.RegisterShield(timestamp, sourceSerial, sourceName, sourceFlags, targetSerial, targetName, spellId, spellSchool, amount, spellName)
			elseif (token == "SPELL_AURA_REMOVED") then
				classicAbsorb.RemoveShield(timestamp, targetSerial, targetName, spellId, sourceName)
			end
		end

		function classicAbsorb.HandleHeal(timestamp, sourceSerial, sourceName, sourceFlags, targetSerial, targetName, targetFlags, targetFlags2, spellId, spellName, spellType, amount)
			classicAbsorb.StoreRecentHeal(timestamp, sourceName, targetName, amount)
		end

		function classicAbsorb.HandleDamage(token, timestamp, sourceSerial, sourceName, sourceFlags, targetSerial, targetName, targetFlags, targetFlags2, ...)
			if (not targetName) then
				return
			end
			local absorbed, school

			if (token == "SWING_DAMAGE") then
				local amount, overkill, swingSchool, resisted, blocked, absorbField = ...
				absorbed = absorbField
				school = swingSchool
			else
				local spellId, spellName, spellSchool, amount, overkill, schoolArg, resisted, blocked, absorbField = ...
				absorbed = absorbField
				school = spellSchool or schoolArg
			end

			if (absorbed and absorbed > 0) then
				classicAbsorb.ApplyAbsorb(timestamp, targetSerial, targetName, targetFlags, targetFlags2, absorbed, school)
			end
		end

		function classicAbsorb.HandleMiss(token, timestamp, sourceSerial, sourceName, sourceFlags, targetSerial, targetName, targetFlags, targetFlags2, ...)
			if (not targetName) then
				return
			end

			local missType, absorbed, school
			if (token == "SWING_MISSED") then
				missType, absorbed = ...
			else
				local spellId, spellName, spellSchool, missTypeArg, absorbedArg = ...
				missType = missTypeArg
				absorbed = absorbedArg
				school = spellSchool
			end

			if (missType == "ABSORB" and absorbed and absorbed > 0) then
				classicAbsorb.ApplyAbsorb(timestamp, targetSerial, targetName, targetFlags, targetFlags2, absorbed, school)
			end
		end

		function classicAbsorb.HandleEnvironment(timestamp, sourceSerial, sourceName, sourceFlags, targetSerial, targetName, targetFlags, targetFlags2, ...)
			if (not targetName) then
				return
			end
			...
			if (absorbed and absorbed > 0) then
				local spellschool = 1
				if (envType == "Fire" or envType == "FIRE" or envType == "Lava" or envType == "LAVA") then
					spellschool = 0x04
				elseif (envType == "Slime" or envType == "SLIME") then
					spellschool = 0x08
				end
				classicAbsorb.ApplyAbsorb(timestamp, targetSerial, targetName, targetFlags, targetFlags2, absorbed, spellschool)
			end
		end

		function classicAbsorb.OnCombatLogEvent(token, timestamp, sourceSerial, sourceName, sourceFlags, targetSerial, targetName, targetFlags, targetFlags2, ...)
			if (not isCLASSIC) then
				return
			end

			if (token == "SPELL_AURA_APPLIED" or token == "SPELL_AURA_REFRESH" or token == "SPELL_AURA_REMOVED") then
				return classicAbsorb.HandleAura(token, timestamp, sourceSerial, sourceName, sourceFlags, targetSerial, targetName, targetFlags, targetFlags2, ...)
			elseif (token == "SPELL_HEAL" or token == "SPELL_PERIODIC_HEAL") then
				return classicAbsorb.HandleHeal(timestamp, sourceSerial, sourceName, sourceFlags, targetSerial, targetName, targetFlags, targetFlags2, ...)
			elseif (token == "SWING_DAMAGE" or token == "RANGE_DAMAGE" or token == "SPELL_DAMAGE" or token == "SPELL_PERIODIC_DAMAGE" or token == "DAMAGE_SHIELD") then
				return classicAbsorb.HandleDamage(token, timestamp, sourceSerial, sourceName, sourceFlags, targetSerial, targetName, targetFlags, targetFlags2, ...)
			elseif (token == "SWING_MISSED" or token == "RANGE_MISSED" or token == "SPELL_MISSED" or token == "SPELL_PERIODIC_MISSED") then
				return classicAbsorb.HandleMiss(token, timestamp, sourceSerial, sourceName, sourceFlags, targetSerial, targetName, targetFlags, targetFlags2, ...)
			elseif (token == "ENVIRONMENTAL_DAMAGE") then
				return classicAbsorb.HandleEnvironment(timestamp, sourceSerial, sourceName, sourceFlags, targetSerial, targetName, targetFlags, targetFlags2, ...)
			end
		end
...
-----------------------------------------------------------------------------------------------------------------------------------------
		...
		if (not Details.in_combat) then --~startcombat ~combatstart
			if (	token ~= "SPELL_PERIODIC_DAMAGE" and
				...
				if (token == "SPELL_PERIODIC_DAMAGE" and sourceName == Details.playername) then
					...
					if (Details.last_combat_time + 5 < _tempo) then
						Details222.StartCombat(sourceSerial, sourceName, sourceFlags, targetSerial, targetName, targetFlags)
					end
				end
			end
	...
	--get actors
		...
		amountMissed = amountMissed or 0
...
-----------------------------------------------------------------------------------------------------------------------------------------
	...
	function parser:heal(token, time, sourceSerial, sourceName, sourceFlags, targetSerial, targetName, targetFlags, targetFlags2, spellId, spellName, spellType, amount, overHealing, absorbed, critical, bIsShield)
		...
		if (not _in_combat) then
			-- allow shield absorbs to register even outside combat so absorbs applied to others are counted
			if (not _in_resting_zone and not bIsShield) then --and not in a resting zone
				return
			end
		end
	...
	--actor target
		if (effectiveHeal > 0) then
			...
			if (bIsShield) then
				...
				--keep damage prevented views in sync
				sourceActor.totalabsorbed = (sourceActor.totalabsorbed or 0) + effectiveHeal
				if (ownerActor) then
					ownerActor.totalabsorbed = (ownerActor.totalabsorbed or 0) + effectiveHeal
				end
			end
...
local SPELL_POWER_PAIN = SPELL_POWER_PAIN or (PowerEnum and PowerEnum.Pain) or 18
	...
	function parser:energize (token, time, sourceSerial, sourceName, sourceFlags, targetSerial, targetName, targetFlags, targetFlags2, spellId, spellName, spellType, amount, overpower, powerType, altpower)
		...
		--older clients (e.g., WotLK) send powerType in the overpower slot and leave powerType nil
		if (not powerType and overpower and (energy_types[overpower] or resourcePowerType[overpower])) then
			powerType, overpower = overpower, 0
		end
...
-----------------------------------------------------------------------------------------------------------------------------------------
	...
	------------------------------------------------------------------------------------------------
		...
		if (raid_members_cache[sourceSerial]) then
			EnsureDefensiveCooldowns()
			--check if is a cooldown
			local cooldownInfo = defensive_cooldowns[spellId]
			if (not cooldownInfo) then
				--fallback for Ascension/custom spellIds: infer a personal cooldown from base cooldown length
				local baseCd = GetSpellBaseCooldown and GetSpellBaseCooldown(spellId)
				if (baseCd and baseCd > 15000) then --longer than 15s; skip gcd/rotational spells
					cooldownInfo = {type = 2}
					defensive_cooldowns[spellId] = cooldownInfo
				end
			end
...
--core
	...
	function Details:CaptureEnable (capture_type)
		...
		capture_type = string.lower(capture_type)
		--sync parser options to ensure toggles changed through the UI are reflected here
	...
	function Details.parser_functions:ADDON_LOADED(...)
		...
		if (addonName == "DetailsWotlkPort" or addonName == "Details") then
			TurnTheSpeakersOn()
		end
	end
	...
	function Details222.Parser.OnParserEvent(_, _, ...)
		local time, token, hidding, who_serial, who_name, who_flags, who_flags2, target_serial, target_name, target_flags, target_flags2, A1, A2, A3, A4, A5, A6, A7, A8, A9, A10, A11, A12 = CombatLogGetCurrentEventInfo(...)
		local func = token_list[token]

		if (isCLASSIC) then
			classicAbsorb.OnCombatLogEvent(token, time, who_serial, who_name, who_flags, target_serial, target_name, target_flags, target_flags2, A1, A2, A3, A4, A5, A6, A7, A8, A9, A10, A11, A12)
		end
	...
	function Details222.Parser.OnParserEventPVP(_, _, ...)
		...
		if (isCLASSIC) then
			classicAbsorb.OnCombatLogEvent(token, time, sourceGUID, sourceName, sourceFlags, targetGUID, targetName, targetFlags, targetFlags2, A1, A2, A3, A4, A5, A6, A7, A8, A9, A10, A11, A12)
		end
	...
	function Details222.Parser.OnParserEventOutOfCombat(_, _, ...)
		local time, token, hidding, who_serial, who_name, who_flags, who_flags2, target_serial, target_name, target_flags, target_flags2, A1, A2, A3, A4, A5, A6, A7, A8, A9, A10, A11, A12 = CombatLogGetCurrentEventInfo(...)
	...
	local parserDebug = {}
	function Details.OnParserEventDebug(_, _, ...)																											    --buffs: spellschool, auraType, amount, arg1, arg2, arg3
