...
--built in announcers
		...
		elseif (channel == "PRINT") then
			...
			if (custom ~= "") then
				...
				if (class) then
					...
					class_color = "|TInterface\\AddOns\\DetailsWotlkPort\\images\\classes_small_alpha:12:12:0:0:128:128:" .. coords[1]/2*128 .. ":" .. coords[2]/2*128 .. ":" .. coords[3]/2*128 .. ":" .. coords[4]/2*128 .. "|t |c" .. RAID_CLASS_COLORS [class].colorStr
				end
	...
	function _detalhes:cooldown_announcer (token, time, who_serial, who_name, who_flags, alvo_serial, alvo_name, alvo_flags, spellid, spellname)
		...
		elseif (channel == "PRINT") then
			...
			if (class2) then
				...
				class_color2 = " -> |TInterface\\AddOns\\DetailsWotlkPort\\images\\classes_small_alpha:12:12:0:0:128:128:" .. coords[1]/2*128 .. ":" .. coords[2]/2*128 .. ":" .. coords[3]/2*128 .. ":" .. coords[4]/2*128 .. "|t |c" .. RAID_CLASS_COLORS [class2].colorStr
	...
	function _detalhes:death_announcer (token, time, who_serial, who_name, who_flags, alvo_serial, alvo_name, alvo_flags, death_table, last_cooldown, death_at, max_health)
			...
			if (where == 4) then --observer
				...
				if (class) then
					local coords = CLASS_ICON_TCOORDS [class]
					class_color = "|TInterface\\AddOns\\DetailsWotlkPort\\images\\classes_small_alpha:12:12:0:0:256:256:" .. coords[1]/2*256 .. ":" .. coords[2]/2*256 .. ":" .. coords[3]/2*256 .. ":" .. coords[4]/2*256 .. "|t |c" .. RAID_CLASS_COLORS [class].colorStr
				end
