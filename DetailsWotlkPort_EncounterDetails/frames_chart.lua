...
function encounterDetails:CreatePhaseTooltip(chartPanel)
    ...
    local damageTexture = detailsFramework:CreateImage(phaseTooltip,[[Interface\AddOns\DetailsWotlkPort\images\skins\classic_skin_v1]], 16, 16, "overlay", {11/1024, 24/1024, 376/1024, 390/1024})
    ...
    local healingTexture = detailsFramework:CreateImage(phaseTooltip,[[Interface\AddOns\DetailsWotlkPort\images\skins\classic_skin_v1]], 16, 16, "overlay", {43/1024, 57/1024, 376/1024, 390/1024})
    ...
    local createTooltipBar = function(index, xOffset)
        ...
        local backgroundTexture = detailsFramework:CreateImage(newtooltipBar, [[Interface\AddOns\DetailsWotlkPort\images\bar_serenity]], nil, nil, "border")
        ...
        statusBarTexture:SetTexture([[Interface\AddOns\DetailsWotlkPort\images\bar_serenity]])
    ...
    local onEnterPhase = function(self)
        ...
        if (combatObject) then
            ...
            for index, playerTable in ipairs(listOfPlayers) do
                ...
                if (spec) then
                    tooltipBar.icon.texture = [[Interface\AddOns\DetailsWotlkPort\images\spec_icons_normal]]
                    tooltipBar.icon.texcoord = encounterDetails.class_specs_coords[spec]

                elseif (class) then
                    tooltipBar.icon.texture = [[Interface\AddOns\DetailsWotlkPort\images\classes_small_alpha]]
            ...
            for index, playerTable in ipairs(listOfPlayersHeal) do
                ...
                elseif (class) then
                    tooltipBar.icon:SetTexture([[Interface\AddOns\DetailsWotlkPort\images\classes_small_alpha]])
            ...
            if (not vRowFrame) then
                ...
                vRowFrame:SetScript("OnEnter", function(frame)
                    if (vRowFrame.dead[1] and vRowFrame.dead[1][3] and vRowFrame.dead[1][3][2]) then
                        ...
                        if (class) then
                            GameCooltip:AddIcon([[Interface\AddOns\DetailsWotlkPort\images\classes_small]], 1, 1, 12, 12, l, r, t, b)
                        end
                        GameCooltip:AddLine("")
                ...
                vRowFrame.texture:SetTexture("Interface\\AddOns\\DetailsWotlkPort\\images\\verticalline")
